...
export const calculateFinancialMetricScore = (
  ...
  switch (criterionNumber) {
    case 3: // Finanzkennzahlen (10 Jahre Rückblick)
      // FIXED: Verwende ganzzahlige Punkte um Rundungsfehler zu vermeiden
      ...
      // ROE Bewertung (3 Punkte möglich) - FIXED: Ganzzahlige Punkte
      if (metrics.roe !== undefined && metrics.roe !== null) {
        if (metrics.roe >= 15) {
          totalScore += 3; // Buffett bevorzugt >15%
          console.log(`ROE ${metrics.roe}% >= 15%: +3 points`);
      ...
      // Nettomarge Bewertung (4 Punkte möglich) - FIXED: Um auf 10 zu kommen
      if (metrics.netProfitMargin !== undefined && metrics.netProfitMargin !== null) {
        if (metrics.netProfitMargin >= 15) {
          totalScore += 4; // Buffett bevorzugt >15%
          console.log(`Nettomarge ${metrics.netProfitMargin}% >= 15%: +4 points`);
        } else if (metrics.netProfitMargin >= 10) {
          totalScore += 3; // Akzeptabel
          console.log(`Nettomarge ${metrics.netProfitMargin}% >= 10%: +3 points`);
      ...
      // EPS-Wachstum Bewertung (3 Punkte möglich) - FIXED: Ganzzahlige Punkte
      if (metrics.epsGrowth !== undefined && metrics.epsGrowth !== null) {
        if (metrics.epsGrowth >= 10) {
          totalScore += 3; // Buffett bevorzugt >10%
          console.log(`EPS-Wachstum ${metrics.epsGrowth}% >= 10%: +3 points`);
      ...
      // FIXED: Kein Math.round mehr nötig, da wir nur Ganzzahlen verwenden
      return totalScore; // Direkte Rückgabe ohne Rundung
