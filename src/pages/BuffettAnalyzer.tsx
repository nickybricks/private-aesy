        ...
        <div className="p-3 sm:p-4 md:p-6 w-full max-w-screen-xl mx-auto">
          ...
          {stockInfo &&
            ...
            needsCurrencyConversion(stockInfo.reportedCurrency, stockInfo.currency) && (
              <CurrencyAlert reportedCurrency={stockInfo.reportedCurrency} stockCurrency={stockInfo.currency} />
            )}
            ...
            <div className="mb-6 sm:mb-8 -mx-3 sm:mx-0 pb-3">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <div className="relative">
                  ...
                  <TabsList
                    className="w-full justify-start h-11 sm:h-12 bg-transparent border-b border-border rounded-none p-0 pl-3 sm:pl-0 overflow-x-auto overflow-y-hidden flex-nowrap scrollbar-hide"
                    style={{
                      scrollbarWidth: "none",
                ...
                <TabsContent value="news" className="mt-4 sm:mt-6 px-3 sm:px-0">
                  <NewsSection newsItems={newsItems} pressReleases={pressReleases} />
                </TabsContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <CurrentRatioCard
                      currentValue={
                        ...
                        financialMetrics?.metrics?.find((m) => m.name === "Current Ratio")?.value ??
                        null
                      }
                    ...
                    <NetDebtToEbitdaCard
                      currentValue={
                        ...
                        financialMetrics?.metrics?.find((m) => m.name === "Net Debt to EBITDA")?.value ??
                        null
                      }
                ...
                <TabsContent value="valuation" className="mt-4 sm:mt-6 px-3 sm:px-0">
                  <div className="space-y-4">
                    ...
                    <PERatioCard
                      ...
                      currentStockPE={
                        financialMetrics?.historicalData?.peRatio?.find((pe) => pe.year.includes("TTM"))?.value
                      }
                      currentIndustryPE={
                    ...
                    {valuationData?.assumptions?.tangibleBookPerShare && (
                      <PriceToBookCard
                        currentPrice={stockInfo.price}
                        bookValuePerShare={
                          financialMetrics?.bookValuePerShare || valuationData.assumptions.tangibleBookPerShare
                        }
                        historicalPrices={
                          financialMetrics?.historicalData?.peRatioWeekly?.map((pe) => {
                ...
                <TabsContent value="growth-rank" className="mt-4 sm:mt-6 px-3 sm:px-0">
                  <div className="space-y-6">
                    {financialMetrics?.historicalData?.revenue && (
                      <RevenueGrowthCard historicalRevenue={financialMetrics.historicalData.revenue} />
                    )}

                    {financialMetrics?.historicalData?.ebitda && (
                      <EbitdaGrowthCard historicalEbitda={financialMetrics.historicalData.ebitda} />
                    )}

                    {financialMetrics?.historicalData?.epsWoNri && (
                      <EpsWoNriGrowthCard historicalEpsWoNri={financialMetrics.historicalData.epsWoNri} />
                    )}

                    {financialMetrics?.historicalData?.freeCashFlow && (
                      <FcfGrowthCard historicalFcf={financialMetrics.historicalData.freeCashFlow} />
                    )}
                  </div>
                </TabsContent>
                  ...
                  {qualitativeScores && deepResearchPerformed ? (
                    ...
                    <Card className="p-6">
                      ...
                      <p className="text-muted-foreground">
                        Deep-Research erforderlich. Bitte führen Sie zuerst eine KI-Analyse durch, um die 8 qualitativen
                        Buffett-Kriterien zu bewerten.
                      </p>
                    </Card>
                  )}
                </TabsContent>
