...
export const PriceToCashFlowCard: React.FC<PriceToCashFlowCardProps> = ({
    ...
    <div className="space-y-3 max-w-md">
      <div>
        ...
        <ul className="text-sm list-disc list-inside mt-1">
          <li>
            <strong>P/FCF</strong>: Preis zum <strong>freien Cashflow</strong> (nach nötigen Investitionen/Capex). →{" "}
            <strong>Bevorzugt!</strong>
          </li>
        </ul>
        <p className="text-sm mt-2 text-muted-foreground italic">
          Beispiel: Kurs 30 €, FCF/Aktie 2,5 € ⇒ <strong>P/FCF 12</strong> (du zahlst das 12-Fache des jährlichen freien
          Cashflows).
        </p>
      </div>
      ...
      <div className="pt-2 border-t">
        ...
        <ul className="text-sm space-y-1 list-disc list-inside">
          <li>
            <strong>Cash schlägt Papiergewinn:</strong> Cashflow ist schwerer „schönzurechnen" als der Gewinn.
          </li>
          <li>
            <strong>Ausschüttungsfähigkeit:</strong> Aus FCF werden Dividenden, Rückkäufe, Schuldenabbau finanziert.
          </li>
          <li>
            <strong>Bewertungsfilter:</strong> Niedrigeres Multiple = mehr Sicherheitsmarge – sofern{" "}
            <strong>qualitativer Cash</strong>.
          </li>
        </ul>
      </div>
    </div>
  ...
  const scoringTooltip = (
    <div className="space-y-1">
      ...
      <p className="text-sm">
        <span className="text-green-600">●</span> 4 Punkte: P/FCF ≤ 12
      </p>
      <p className="text-sm">
        <span className="text-yellow-600">●</span> 3 Punkte: P/FCF 12-16
      </p>
      <p className="text-sm">
        <span className="text-orange-600">●</span> 2 Punkte: P/FCF 16-20
      </p>
      <p className="text-sm">
        <span className="text-orange-600">●</span> 1 Punkt: P/FCF 20-25
      </p>
      <p className="text-sm">
        <span className="text-red-600">●</span> 0 Punkte: P/FCF &gt; 25 oder FCF in 2/3 Jahren negativ
      </p>
    </div>
  );
  ...
  if (!fcfPerShare || fcfPerShare <= 0) {
    return (
      <Card className="p-4 border-2 bg-gray-50 border-gray-200">
        ...
        <p className="text-sm text-muted-foreground">Free Cashflow je Aktie nicht verfügbar oder FCF ≤ 0</p>
      </Card>
    );
  }
    ...
    <Card className={`p-4 border-2 ${getBgColorByScore(score)}`}>
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="text-right">
          <div className={`text-2xl font-bold ${getColorByScore(score)}`}>{priceToCashFlow?.toFixed(2) || "N/A"}</div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
        ...
        <div>
          ...
          <p className="font-semibold">
            {fcfPerShare.toFixed(2)} {currency}
          </p>
        </div>
        <div>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="cursor-help">
                P/FCF{" "}
                <span className={`font-bold ${getColorByScore(score)}`}>{priceToCashFlow?.toFixed(2) || "N/A"}</span>
              </span>
            </TooltipTrigger>
            <TooltipContent>
                        ...
                        <div className="bg-background/95 backdrop-blur-sm border rounded-lg shadow-lg p-3">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-xs">
                              ...
                              <span className="font-semibold">
                                {data.fcfPerShare.toFixed(2)} {currency}
                              </span>
                            </p>
                          </div>
                        </div>
