...
export const ROACard: React.FC<ROACardProps> = ({
  currentValue,
  historicalData,
  preset = "Default",
  scoreFromBackend,
  ...
  const getTooltipContent = () => {
    const baseContent = (
      <>
        ...
        <p className="text-muted-foreground">
          Zeigt, wie viel <strong>Gewinn</strong> ein Unternehmen mit{" "}
          <strong>allen eingesetzten Vermögenswerten</strong> erwirtschaftet (Maschinen, Lager, Software, Cash …).
        </p>
        <p className="text-muted-foreground mt-2">
          Bildlich: Du besitzt Anlagen im Wert von 100 €. Verdient die Firma 8 € pro Jahr, ist die{" "}
          <strong>ROA = 8 %</strong>.
        </p>
        ...
        <div className="mt-2">
          ...
          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
            <li>
              <strong>Effizienz-Gesamtblick:</strong> Misst, wie gut <em>alle</em> Ressourcen arbeiten – nicht nur das
              Eigenkapital.
            </li>
            <li>
              <strong>Branchen-Thermometer:</strong> Kapitalleichte Firmen (Software) sollten meist höhere ROA haben als
              kapitalintensive (Industrie).
            </li>
            <li>
              <strong>Qualität der Gewinne:</strong> Stetig solide ROA über Jahre deutet auf gutes Management und
              Prozesse hin.
            </li>
          </ul>
        </div>
        ...
        <div className="mt-2">
          ...
          <p className="text-muted-foreground">
            <strong>10/5/3-Jahres-Median</strong> und <strong>Trend</strong> ansehen (stabil/steigend besser als
            zackig/volatil).
          </p>
        </div>
        ...
        <div className="mt-2">
          ...
          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
            <li>
              <strong className="text-green-600">Grün (stark):</strong> <strong>≥ 8 %</strong> über mehrere Jahre (außer
              sehr kapitalintensiven Sektoren).
            </li>
            <li>
              <strong className="text-yellow-600">Gelb (ok):</strong> <strong>4–8 %</strong> oder stark schwankend.
            </li>
            <li>
              <strong className="text-red-600">Rot (schwach):</strong> <strong>&lt; 4 %</strong> dauerhaft.
            </li>
          </ul>
          <p className="text-muted-foreground text-xs mt-2">
            <em>
              (Immer mit dem <strong>Sektor-Median</strong> spiegeln: in kapitalarmen Sektoren strenger, in
              kapitalintensiven etwas milder.)
            </em>
          </p>
        </div>
      </>
    ...
    // Default
    return <div className="space-y-3 text-sm">{baseContent}</div>;
  };
  ...
  const getScoringTooltip = () => {
    if (preset === "Industrials") {
      return (
        <div className="space-y-1">
          ...
          <p className="text-sm">
            <span className="text-green-600">●</span> 2 Pkt: ≥ 7%
          </p>
          <p className="text-sm">
            <span className="text-yellow-600">●</span> 1 Pkt: 5–&lt;7%
          </p>
          <p className="text-sm">
    ...
    if (preset === "Software") {
      return (
        <div className="space-y-1">
          ...
          <p className="text-sm">
            <span className="text-green-600">●</span> 2 Pkt: ≥ 10%
          </p>
          <p className="text-sm">
            <span className="text-yellow-600">●</span> 1 Pkt: 8–&lt;10%
          </p>
          <p className="text-sm">
            <span className="text-red-600">●</span> 0 Pkt: &lt; 8%
          </p>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-1">
        ...
        <p className="text-sm">
          <span className="text-green-600">●</span> 1 Pkt: ≥ 8%
        </p>
        <p className="text-sm">
          <span className="text-red-600">●</span> 0 Pkt: &lt; 8%
        </p>
    ...
    <Card className={`border ${getBgColor(currentValue)}`}>
      ...
      <CardContent>
        ...
        {historicalData && historicalData.length > 0 ? (
          <>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={chartData}>
                ...
                <YAxis tick={{ fill: "#666", fontSize: 12 }} tickFormatter={(value) => `${value}%`} />
                <RechartsTooltip
          ...
          <div className="text-center text-muted-foreground text-sm py-8">Keine historischen Daten verfügbar</div>
        )}
      </CardContent>
    </Card>
  );
