...
export const PriceToMedianPSChart: React.FC<PriceToMedianPSChartProps> = ({
  ...
  useEffect(() => {
    ...
    const median = psValues.length % 2 === 0 ? (psValues[mid - 1] + psValues[mid]) / 2 : psValues[mid];
  ...
  const findQuarterForDate = (dailyDate: string, quarterMap: Map<string, QuarterData>): QuarterData | undefined => {
    ...
    <div className="space-y-3 max-w-md">
      <div>
        ...
        <p className="text-xs mt-1">
          Zeigt, zu welchem Kurs die Aktie handeln würde, <strong>wenn</strong> sie heute genau zum{" "}
          <strong>Median ihres historischen KUV (P/S)</strong> bewertet wäre.
        </p>
      </div>
      ...
      <div className="text-xs">
        ...
        <ul className="list-disc list-inside space-y-1 pl-2">
          <li>
            <strong>KUV (P/S)</strong> = Price-to-Sales Ratio = Kurs / Umsatz je Aktie
          </li>
          <li>
            <strong>RPS</strong> = Revenue per Share = Umsatz je Aktie (TTM)
          </li>
          <li>
            <strong>Median-KUV</strong> = Median aller KUV-Werte im Lookback-Fenster
          </li>
        </ul>
      </div>
      ...
      <div className="text-xs space-y-1">
        ...
        <div className="space-y-1 pl-2 border-l-2 border-border">
          <p>
            • <strong>Kurs &gt; Preis beim Median-KUV</strong> → teurer als historischer Median
          </p>
          <p>
            • <strong>Kurs &lt; Preis beim Median-KUV</strong> → günstiger als historischer Median
          </p>
        </div>
      </div>
    </div>
  ...
  const scoringTooltip = (
    <div className="space-y-1">
      ...
      <p className="text-sm">
        <span className="text-green-600">●</span> 3 Punkte: ≥ 35%
      </p>
      <p className="text-sm">
        <span className="text-lime-600">●</span> 2 Punkte: ≥ 23%
      </p>
      <p className="text-sm">
        <span className="text-yellow-600">●</span> 1 Punkt: ≥ 12%
      </p>
      <p className="text-sm">
        <span className="text-orange-600">●</span> 0,5 Punkt: ≥ 0%
      </p>
      <p className="text-sm">
        <span className="text-red-600">●</span> 0 Punkte: &lt; 0% (überbewertet)
      </p>
    </div>
  );
    ...
    return (
      <Card className="p-4 border-2 border-yellow-200 bg-yellow-50">
        <div className="flex items-center gap-2 text-yellow-700">
          ...
          <p className="text-sm font-medium">Nicht genügend Daten für P/S-Analyse (min. 2 Jahre benötigt)</p>
        </div>
      </Card>
    );
  }
    ...
    <Card className={`p-4 border-2 ${getBgColorByScore(score, maxScore)}`}>
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="text-right">
          ...
          <div className="text-xs text-muted-foreground">{discount >= 0 ? "Unterbewertet" : "Überbewertet"}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
        ...
        <div>
          ...
          <p className="font-semibold">
            {priceAtMedianPS.toFixed(2)} {currency}
          </p>
        </div>
        <div>
          ...
          >
            ...
            {discount >= 0 ? "+" : ""}
            {(priceAtMedianPS - currentPrice).toFixed(2)} {currency}
          </div>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={chartData}>
            ...
            <XAxis
              ...
              stroke="#9ca3af"
            />
            <YAxis tick={{ fontSize: 10 }} stroke="#9ca3af" domain={["auto", "auto"]} width={60} />
            <RechartsTooltip
                  ...
                  if (data.priceAtMedianPS && data.rps && data.ps) {
                    ...
                    return (
                      <div className="bg-popover text-popover-foreground border border-border rounded-lg shadow-lg p-3 max-w-[280px]">
                        <p className="text-xs font-semibold mb-1">{new Date(data.date).toLocaleDateString("de-DE")}</p>
                        ...
                        <p className="text-sm text-purple-600">
                          Preis @ Median P/S:{" "}
                          <span className="font-bold">
                            {data.priceAtMedianPS.toFixed(2)} {currency}
                          </span>
                        </p>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Discount:{" "}
                          <span className={`font-bold ${discountAtPoint >= 0 ? "text-green-600" : "text-destructive"}`}>
                            {discountAtPoint >= 0 ? "+" : ""}
                            {discountAtPoint.toFixed(1)}%
                          </span>
                        </p>
                      </div>
                    );
            ...
            <Line
              ...
              dot={(props: any) => {
                ...
                if (payload.priceAtMedianPS !== undefined) {
                  return <circle cx={cx} cy={cy} r={3} fill="#8b5cf6" stroke="#8b5cf6" strokeWidth={1} />;
                }
                return null;
              }}
              connectNulls={true}
