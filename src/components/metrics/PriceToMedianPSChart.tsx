...
export const PriceToMedianPSChart: React.FC<PriceToMedianPSChartProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setDailyPrices(dailyPriceData.reverse()); // Oldest first
  ...
  const filterDailyPricesByRange = (data: {date: string; price: number}[], range: TimeRange) => {
    if (!data || data.length === 0) return [];
  ...
  // Create quarter date lookup for efficient checking
  const quarterDatesSet = new Set(quarterData.map(q => q.date));
  const quarterDataMap = new Map(quarterData.map(q => [q.date, q]));

  // Filter data by range
  const filteredQuarterData = filterDataByRange(quarterData, selectedRange);
  const filteredDailyPrices = filterDailyPricesByRange(dailyPrices, selectedRange);

  // Combine daily prices with quarterly P/S data
  const chartData = filteredDailyPrices.map(daily => {
    const quarterInfo = quarterDataMap.get(daily.date);
    return {
      date: daily.date,
      price: daily.price,
      priceAtMedianPS: quarterInfo ? quarterInfo.rps * medianPS : undefined,
      rps: quarterInfo?.rps,
      ps: quarterInfo?.ps,
      isQuarterEnd: !!quarterInfo
    };
  });
            ...
            <RechartsTooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  ...
                  const hasQuarterData = data.isQuarterEnd;
                  ...
                  return (
                    <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
                      ...
                      {!hasQuarterData && (
                        <p className="text-xs text-muted-foreground mt-1">
                          (Kein Quartalsende - P/S-Daten nicht verfügbar)
                        </p>
                      )}
                    </div>
                  );
