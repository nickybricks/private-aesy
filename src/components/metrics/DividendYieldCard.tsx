...
export const DividendYieldCard: React.FC<DividendYieldCardProps> = ({
  ...
  const dividendYield =
    currentPrice > 0 && currentDividendPerShare > 0 ? (currentDividendPerShare / currentPrice) * 100 : 0;
  ...
  const latestPayoutRatioFCF =
    payoutRatioFCFHistory.length > 0 ? payoutRatioFCFHistory[payoutRatioFCFHistory.length - 1].value : null;

  const latestPayoutRatioEPS =
    payoutRatioEPSHistory.length > 0 ? payoutRatioEPSHistory[payoutRatioEPSHistory.length - 1].value : null;
    ...
    <div className="max-w-sm space-y-2">
      ...
      <div className="space-y-1">
        ...
        <ul className="text-sm space-y-1 list-disc list-inside">
          <li>
            <strong>Einkommensquelle:</strong> Regelmäßiger Cash-Rückfluss für Anleger.
          </li>
          <li>
            <strong>Bewertungshinweis:</strong> Höhere Rendite kann auf günstige Bewertung hindeuten – oder auf
            Probleme.
          </li>
          <li>
            <strong>Disziplin-Signal:</strong> Stetige, wachsende Dividenden deuten oft auf solide Cashflows hin.
          </li>
        </ul>
      </div>
  ...
  const scoringTooltip = (
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs">
          <span className="text-green-600">●</span> 40-65% → 1 Punkt
        </p>
        <p className="text-xs">
          <span className="text-yellow-600">●</span> ≤ 40% → 0,5 Punkte (konservativ, Wachstumsraum)
        </p>
        <p className="text-xs">
          <span className="text-yellow-600">●</span> 65-80% → 0,5 Punkte (ok, weniger Puffer)
        </p>
        <p className="text-xs">
          <span className="text-red-600">●</span> &gt; 80% oder negativ → 0 Punkte
        </p>
      </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs">
          <span className="text-green-600">●</span> ≥ 10 Jahre → 1 Punkt
        </p>
        <p className="text-xs">
          <span className="text-yellow-600">●</span> 5-&lt;10 Jahre → 0,66 Punkte
        </p>
        <p className="text-xs">
          <span className="text-orange-600">●</span> &lt; 5 Jahre → 0,34 Punkte
        </p>
        <p className="text-xs">
          <span className="text-red-600">●</span> Cut in 5J → 0 Punkte
        </p>

        <p className="text-xs font-medium mt-1">CAGR (3/5/10J, Median):</p>
        <p className="text-xs">
          <span className="text-green-600">●</span> ≥ 6% p.a. → 1 Punkt
        </p>
        <p className="text-xs">
          <span className="text-yellow-600">●</span> 3-&lt;6% → 0,66 Punkte
        </p>
        <p className="text-xs">
          <span className="text-orange-600">●</span> 0-&lt;3% → 0,34 Punkte
        </p>
        <p className="text-xs">
          <span className="text-red-600">●</span> &lt; 0% → 0 Punkte
        </p>
      </div>
    </div>
  );
    ...
    <Card className={`p-4 border-2 ${getBgColorByRatio(totalScore, maxScore)}`}>
      ...
      <div className="grid grid-cols-4 gap-3 mb-4 text-sm">
        ...
        <div>
          ...
          <p className="font-semibold">{medianCAGR !== null ? `${medianCAGR.toFixed(1)}%` : "N/A"}</p>
        </div>
      </div>
      ...
      {mergedChartData.length > 1 && (
        <div className="mt-4">
                    ...
                    return (
                      <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
                        ...
                        <p className="text-sm text-blue-600">
                          Dividende:{" "}
                          <span className="font-bold">${payload[0].payload.dividend?.toFixed(2) || "N/A"}</span>
                        </p>
                        <p className="text-sm text-emerald-600">
                          Payout (EPS):{" "}
                          <span className="font-bold">{payload[0].payload.payoutRatioEPS?.toFixed(1) || "N/A"}%</span>
                        </p>
                        <p className="text-sm text-sky-600">
                          Payout (FCF):{" "}
                          <span className="font-bold">{payload[0].payload.payoutRatioFCF?.toFixed(1) || "N/A"}%</span>
                        </p>
                      </div>
                    );
                  }
          ...
          <div className="flex justify-center gap-4 mt-2 text-xs text-muted-foreground">
            <span>
              <span className="text-green-600">---</span> 40% (Min. Ideal)
            </span>
            <span>
              <span className="text-yellow-600">---</span> 65% (Max. Ideal)
            </span>
            <span>
              <span className="text-orange-600">---</span> 80% (Risiko)
            </span>
          </div>
          <div className="flex justify-center gap-4 mt-1 text-xs text-muted-foreground">
            <span>
              <span className="inline-block w-3 h-3 bg-emerald-500 mr-1"></span>EPS-basiert
            </span>
            <span>
              <span className="inline-block w-3 h-3 bg-blue-500 mr-1"></span>FCF-basiert
            </span>
          </div>
        </div>
      )}
