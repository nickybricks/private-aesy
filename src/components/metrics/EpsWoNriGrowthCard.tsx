...
export function EpsWoNriGrowthCard({ historicalEpsWoNri }: EpsWoNriGrowthCardProps) {
  ...
  const cagrData = useMemo(() => {
    ...
    const sortedData = [...historicalEpsWoNri].sort((a, b) => parseInt(a.year) - parseInt(b.year));
  ...
  console.log("🎯 EPS w/o NRI Card: Primary CAGR Selection", {
    ...
    priority:
      cagrData.cagr10y !== null ? "10Y" : cagrData.cagr5y !== null ? "5Y" : cagrData.cagr3y !== null ? "3Y" : "None",
    score,
    maxScore,
  });
  ...
  const chartData = useMemo(() => {
    ...
    const data = filteredData.map((item, index) => {
      const growthRate =
        index > 0 && filteredData[index - 1].value > 0
    ...
    console.log("📈 EPS w/o NRI Card: Chart Data Prepared", {
      ...
      avgGrowthRate:
        data.length > 1
          ? (
              data.reduce((sum, d) => sum + (d.growthRate || 0), 0) / data.filter((d) => d.growthRate !== null).length
            ).toFixed(2) + "%"
          : null,
    });
    ...
    <div className="space-y-3 max-w-md">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          (EPS without NRI Growth = Earnings per Share without Non-Recurring Items Growth)
        </p>
      ...
      <div className="text-xs">
        <p className="mb-2">
          Die <strong>Growth Rate</strong> zeigt, wie stark dieser <strong>bereinigte Gewinn je Aktie</strong> über die
          Zeit wächst (YoY oder als <strong>CAGR</strong> über 3/5/10 Jahre).
        </p>
      ...
      <div className="text-xs">
        ...
        <ul className="list-disc list-inside space-y-1 pl-2">
          <li>
            <strong>Saubere Ertragskraft:</strong> Blendet Einmaleffekte aus → <strong>ehrliches</strong>{" "}
            Gewinnwachstum.
          </li>
          <li>
            <strong>Aktionärssicht:</strong> "Je Aktie" berücksichtigt Verwässerung/Rückkäufe.
          </li>
          <li>
            <strong>Bewertung & Moat:</strong> Stabiles EPS-Wachstum stützt <strong>Wert</strong> (P/E), besonders wenn{" "}
            <strong>ROIC &gt; WACC</strong>.
          </li>
        </ul>
      </div>
    </div>
  ...
  const scoringTooltip = (
    <div className="space-y-1">
      ...
      <p className="text-sm">
        <span className="text-success">●</span> 6 Punkte: ≥ 15%
      </p>
      <p className="text-sm">
        <span className="text-chart-2">●</span> 5 Punkte: 12–&lt;15%
      </p>
      <p className="text-sm">
        <span className="text-chart-2">●</span> 4 Punkte: 9–&lt;12%
      </p>
      <p className="text-sm">
        <span className="text-warning">●</span> 2 Punkte: 6–&lt;9%
      </p>
      <p className="text-sm">
  ...
  if (!historicalEpsWoNri || historicalEpsWoNri.length < 2) {
    return (
      <Card className="p-4 border-2 border-muted">
        ...
        <p className="text-sm text-muted-foreground">Nicht genügend historische EPS w/o NRI-Daten verfügbar.</p>
      </Card>
    );
  }
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
        {cagrData.cagr3y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr3y >= 9 ? "text-green-600" : cagrData.cagr3y >= 6 ? "text-yellow-600" : ""}`}
            >
        ...
        {cagrData.cagr5y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr5y >= 9 ? "text-green-600" : cagrData.cagr5y >= 6 ? "text-yellow-600" : ""}`}
            >
        ...
        {cagrData.cagr10y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr10y >= 9 ? "text-green-600" : cagrData.cagr10y >= 6 ? "text-yellow-600" : ""}`}
            >
                    ...
                    <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
                      ...
                      <p className="text-sm text-blue-600">
                        ...
                        <span className="font-bold">
                          {data.value.toFixed(2)} {stockCurrency}
                        </span>
                      </p>
