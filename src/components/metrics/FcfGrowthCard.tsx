...
export const FcfGrowthCard: React.FC<FcfGrowthCardProps> = ({ historicalFcf }) => {
  ...
  const cagrData = useMemo(() => {
    ...
    const sortedData = [...historicalFcf].sort((a, b) => parseInt(a.year) - parseInt(b.year));
  ...
  const mainTooltipContent = (
    <div className="space-y-3 max-w-md">
      ...
      <div className="text-xs">
        <p className="mb-2">
          Die <strong>FCF Growth Rate</strong> zeigt, wie stark der <strong>freie Cash</strong> eines Unternehmens über
          die Zeit steigt (YoY oder als <strong>CAGR</strong> über 3/5/10 Jahre).
        </p>
      ...
      <div className="text-xs">
        ...
        <ul className="list-disc list-inside space-y-1 pl-2">
          <li>
            <strong>Ausschüttungs-Power:</strong> Aus FCF werden <strong>Dividenden</strong>, <strong>Rückkäufe</strong>{" "}
            und <strong>Schuldenabbau</strong> bezahlt.
          </li>
          <li>
            <strong>Qualitätsmaß:</strong> Cash ist schwerer zu „schönen" als Buchgewinne.
          </li>
          <li>
            <strong>Bewertung:</strong> Steigender, stabiler FCF stützt <strong>P/FCF</strong>, <strong>EV/EBIT</strong>{" "}
            und den <strong>inneren Wert</strong>.
          </li>
        </ul>
      </div>
    </div>
  ...
  const scoringTooltip = (
    <div className="space-y-1">
      ...
      <p className="text-sm">
        <span className="text-success">●</span> 6 Punkte: ≥ 12%
      </p>
      <p className="text-sm">
        <span className="text-chart-2">●</span> 5 Punkte: 10–&lt;12%
      </p>
      <p className="text-sm">
        <span className="text-chart-2">●</span> 4 Punkte: 7–&lt;10%
      </p>
      <p className="text-sm">
        <span className="text-warning">●</span> 2 Punkte: 4–&lt;7%
      </p>
      <p className="text-sm">
        <span className="text-chart-4">●</span> 1 Punkt: 2–&lt;4%
      </p>
      <p className="text-sm">
        <span className="text-destructive">●</span> 0 Punkte: &lt; 2%
      </p>
    </div>
  );
  ...
  if (!historicalFcf || historicalFcf.length < 2) {
    return (
      <Card className="p-4 border-2 border-muted">
        ...
        <p className="text-sm text-muted-foreground">Nicht genügend historische FCF-Daten verfügbar.</p>
      </Card>
    );
  }
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
        {cagrData.cagr3Y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr3Y >= 7 ? "text-green-600" : cagrData.cagr3Y >= 4 ? "text-yellow-600" : ""}`}
            >
        ...
        {cagrData.cagr5Y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr5Y >= 7 ? "text-green-600" : cagrData.cagr5Y >= 4 ? "text-yellow-600" : ""}`}
            >
        ...
        {cagrData.cagr10Y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr10Y >= 7 ? "text-green-600" : cagrData.cagr10Y >= 4 ? "text-yellow-600" : ""}`}
            >
                    ...
                    <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
                      ...
                      <p className="text-sm text-blue-600">
                        ...
                        <span className="font-bold">
                          {data.fcf.toFixed(2)} Mio. {stockCurrency}
                        </span>
                      </p>
