...
export const RevenueGrowthCard: React.FC<RevenueGrowthCardProps> = ({ historicalRevenue }) => {
  ...
  const cagrData = useMemo(() => {
    ...
    const sortedData = [...historicalRevenue].sort((a, b) => parseInt(a.year) - parseInt(b.year));
    ...
    <div className="space-y-3 max-w-md">
      ...
      <div className="text-xs">
        ...
        <ul className="list-disc list-inside space-y-1 pl-2">
          <li>
            <strong>YoY</strong> (Jahr-zu-Jahr) – kurzfristiger Blick.
          </li>
          <li>
            <strong>CAGR</strong> (durchschnittliche jährliche Wachstumsrate über 3/5/10 Jahre) –{" "}
            <strong>stabilerer</strong> Blick.
          </li>
        </ul>
      ...
      <div className="text-xs">
        ...
        <ul className="list-disc list-inside space-y-1 pl-2">
          <li>
            <strong>Größenmotor:</strong> Ohne Umsatzwachstum ist nachhaltiges Gewinn-/Cash-Wachstum schwer.
          </li>
          <li>
            <strong>Preissetzungsmacht:</strong> Wachstum aus <strong>Preis</strong> + <strong>Menge</strong> zeigt
            Marktmacht.
          </li>
          <li>
            <strong>Skalierung:</strong> Langanhaltendes Wachstum erlaubt Margenhebel.
          </li>
        </ul>
      </div>
    </div>
  ...
  const scoringTooltip = (
    <div className="space-y-1">
      ...
      <p className="text-sm">
        <span className="text-warning">●</span> 2 Punkte: 5–&lt;8%
      </p>
      <p className="text-sm">
        <span className="text-chart-4">●</span> 1 Punkt: 2–&lt;5%
      </p>
      <p className="text-sm">
  ...
  if (!historicalRevenue || historicalRevenue.length < 2) {
    return (
      <Card className="p-4 border-2 border-muted">
        ...
        <p className="text-sm text-muted-foreground">Nicht genügend historische Umsatzdaten verfügbar.</p>
      </Card>
    );
  }
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
        {cagrData.cagr3Y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr3Y >= 8 ? "text-green-600" : cagrData.cagr3Y >= 5 ? "text-yellow-600" : ""}`}
            >
        ...
        {cagrData.cagr5Y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr5Y >= 8 ? "text-green-600" : cagrData.cagr5Y >= 5 ? "text-yellow-600" : ""}`}
            >
        ...
        {cagrData.cagr10Y !== null && (
          <div>
            ...
            <p
              className={`font-semibold ${cagrData.cagr10Y >= 8 ? "text-green-600" : cagrData.cagr10Y >= 5 ? "text-yellow-600" : ""}`}
            >
                    ...
                    <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
                      ...
                      <p className="text-sm text-blue-600">
                        ...
                        <span className="font-bold">
                          {data.revenue.toFixed(2)} Mio. {stockCurrency}
                        </span>
                      </p>
