...
export const PERatioCard: React.FC<PERatioCardProps> = ({
  ...
  const formatScore = (n: number, decimals = 1) => (Number.isInteger(n) ? n.toString() : n.toFixed(decimals));
  ...
  const getRelativeScore = (stockPE: number | null | undefined, industryPE: number | null | undefined): number => {
    ...
    if (peRel <= 0.8) return 1.5; // Significant discount
    if (peRel <= 1.1) return 1.0; // Fair valuation
    if (peRel <= 1.3) return 0.5; // Slight premium
  ...
  const getXAxisTicks = (data: any[], range: TimeRange): string[] => {
    ...
    return data.filter((_, index) => index % interval === 0).map((d) => d.date);
  };
    ...
    <div className="space-y-3 max-w-md">
      <div>
        ...
        <p className="text-xs mt-1">
          Das KGV zeigt, wie teuer eine Aktie im Verhältnis zu ihren Gewinnen ist. Ein niedriges KGV deutet auf eine
          günstige Bewertung hin.
        </p>
      </div>

      <div className="text-xs">
        <p className="font-medium mb-1">Formel:</p>
        <code className="bg-muted px-2 py-1 rounded text-[10px]">KGV = Aktienkurs / Gewinn je Aktie (EPS)</code>
      </div>
      ...
      <div className="text-xs space-y-1">
        ...
        <div className="space-y-1 pl-2 border-l-2 border-border">
          <p>
            • KGV ≤ 15: <strong>Günstig bewertet</strong>
          </p>
          <p>
            • KGV 15-20: <strong>Fair bewertet</strong>
          </p>
          <p>
            • KGV 20-25: <strong>Leicht teuer</strong>
          </p>
          <p>
            • KGV &gt; 25: <strong>Teuer</strong>
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        ...
        <div className="text-xs space-y-0.5">
          <p>
            <span className="text-green-600">●</span> ≤15: 1,5 Punkte
          </p>
          <p>
            <span className="text-yellow-600">●</span> 15-20: 1,0 Punkt
          </p>
          <p>
            <span className="text-orange-600">●</span> 20-25: 0,5 Punkte
          </p>
          <p>
            <span className="text-red-600">●</span> &gt;25: 0,0 Punkte
          </p>
        </div>
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="text-xs space-y-0.5">
          <p>
            <span className="text-green-600">●</span> ≤0,80x: 1,5 Punkte (Discount)
          </p>
          <p>
            <span className="text-yellow-600">●</span> 0,80-1,10x: 1,0 Punkt (Fair)
          </p>
          <p>
            <span className="text-orange-600">●</span> 1,10-1,30x: 0,5 Punkte (Premium)
          </p>
          <p>
            <span className="text-red-600">●</span> &gt;1,30x: 0,0 Punkte (Overpremium)
          </p>
        </div>
      </div>
    </div>
  ...
  const relativeValue = currentStockPE && currentIndustryPE ? currentStockPE / currentIndustryPE : null;
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="cursor-help">
                Absolut-Score:{" "}
                <span className={`font-bold ${getColorByScore(absoluteScore, 1.5)}`}>
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="cursor-help">
                Relativ-Score:{" "}
                <span className={`font-bold ${getColorByScore(relativeScore, 1.5)}`}>
          ...
          <>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="cursor-help">
                    Bewertung vs. Branche:{" "}
                    <span
                      className={`font-bold ${
                        relativeValue && relativeValue <= 0.8
                          ? "text-green-600"
                          : relativeValue && relativeValue <= 1.1
                            ? "text-yellow-600"
                            : "text-red-600"
                      }`}
                    >
                ...
                <TooltipContent>
                  <p className="text-xs">
                    ...
                    {relativeValue &&
                      relativeValue > 0.8 &&
                      relativeValue <= 1.1 &&
                      "Aktie ist fair zur Branche bewertet"}
          ...
          {filteredData.length > 1 && (
            <div className="mt-4">
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={filteredData}>
                  ...
                  <XAxis
                    ...
                    stroke="#9ca3af"
                  />
                  <YAxis tick={{ fontSize: 10 }} stroke="#9ca3af" domain={[0, "auto"]} width={60} />
                  <RechartsTooltip
                  ...
                  {/* Stock P/E Line (blue) */}
                  <Line type="monotone" dataKey="stockPE" stroke="#2563eb" strokeWidth={2.5} dot={false} name="Aktie" />
                </LineChart>
              </ResponsiveContainer>
              ...
              <div className="flex justify-center gap-4 mt-3 text-xs text-muted-foreground">
                ...
                <span>
                  <span className="text-green-600">---</span> 15 (Günstig)
                </span>
                <span>
                  <span className="text-yellow-600">---</span> 20 (Fair)
                </span>
              </div>
            </div>
          )}
